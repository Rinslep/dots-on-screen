...
class Node(object):
    ...
    def __init__(self, window: tk.Canvas, center: int, radius: int, teamid: int) -> object: #, item, value: float
        self.window = window
    ...
    def move(self, e):
        # todo fix this - should move the node.py and its connections slightly closer together
    ...
    def create_node(self, shape, colour):
        shapes = {'oval' : lambda x: Node.create_oval_node(x, fill=colour),
                  'square': lambda x: Node.create_square_node(x, fill=colour)}
